...
public class ServerWarehouseDB extends ServerSeasonalDB 
{
	...
	{
		List<ONCWarehouseVolunteer> volList = warehouseDB.get(DBManager.offset(year)).getList();
	...
	{
		WarehouseDBYear whDBYear = warehouseDB.get(DBManager.offset(year));
		...
		Gson gson = new Gson();
		clientMgr.notifyAllInYearClients(year, "ADDED_WAREHOUSE_VOLUNTEER" + gson.toJson(addedVol, ONCVolunteer.class));
	}
	...
	{
		WarehouseDBYear warehouseDBYear = warehouseDB.get(DBManager.offset(year));
