...
public class ServerBatteryDB extends ServerSeasonalDB
{
	...
	String getBatteries(int year)
	{
		...
		String response = gson.toJson(batteryDB.get(DBManager.offset(year)).getList(), listOfBatteriess);
		return response;	
	}
	...
	String add(int year, String batteryjson, ONCUser client)
	{
		...
		BatteryDBYear batteryDBYear = batteryDB.get(DBManager.offset(year));
	...
	void save(int year)
	{
		...
		{
			String[] header = {"Battery ID", "Child ID", "Gift", "Size", "Quantity"};
